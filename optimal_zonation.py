...
class OptimalZonation(Problem):
    def __init__(self,_G,sn_id,_branch_lodf, _prob_id, _consider_lodf=False, partition_size_upper = 40, partition_size_lower=0, min_nodes_partition = 1):
        ...
        nx.write_gpickle(self.G, str(_prob_id)+'.gpickle')
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for d in range(x.shape[0]):
            threads_f1[d] = threading.Thread(target=self.compute_metric_cyber_infrastructure, args=(x[d], res_f1, d))
            threads_f1[d].start()
        for d in range(x.shape[0]):
            threads_f1[d].join()
        ...
        res_f2 = {}
        for d in range(x.shape[0]):
            threads_f2[d] = threading.Thread(target=self.compute_si, args=(x[d],res_f2, d))
            threads_f2[d].start()
        for d in range(x.shape[0]):
            threads_f2[d].join()
        for i in sorted(res_f2.keys()):
            f2.append(1000/res_f2[i])
    ...
    def compute_metric_cyber_infrastructure(self,sol,res,ix):
        edge_index = np.where(sol == 1)
        temp_graph = self.G.copy()
        # get the edge index from original graph
        edge_list = list(self.G.edges)
        ...
        for sg in enumerate(subgraph):
            ...
            if True in found:
                firewalls += len(list(sg[1].nodes))
            else:
                firewalls += 1
        #
        #
        # for i in range(len(list(self.G.nodes)) - len(subgraph)):
        #     firewalls += 1
        res[ix] = firewalls

    def compute_si(self,sol,res,ix):
        edge_index = np.where(sol == 1)
        temp_graph = self.G.copy()
        # get the edge index from original graph
        edge_list = list(self.G.edges)
        ...
        if self.consider_lodf:
            if relay_info['lodf']!=0.0:
                si_cluster+=(1/10 * relay_info['lodf'])
