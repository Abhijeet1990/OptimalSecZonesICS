...
# this code is for the last figure in paper (Fig. 14)
...
for result in results:
    ...
    # compute the mesh_topo_firewall
    for (node,val) in G.degree():
        if 'ucc' not in node:
            mesh_topo_firewall+=val
            nbrs = [n for n in G.neighbors(node)]
            found = ['ucc' in sub for sub in nbrs]
            if True in found:
                mesh_topo_acls += 8*val
            else:
                mesh_topo_acls += 6*val
        # else:
        #     mesh_topo_acls += 2*val
...
print(mesh_topo_firewall)
print(mesh_topo_acls)

# Subnet Distribution
hist, edges = np.histogram(graph_sizes, density=False, bins=[0,5,10,15,20,25,30,35,40,45,50])

x = np.linspace(0, 50, 11)
p1 = make_plot("Network Size Distribution", hist, edges, 'Subgraph Size','Frequency')

comp_times.sort()
t = np.linspace(min(comp_times), comp_times[-2], 10)
hist2,edges2 = np.histogram(comp_times, density=False, bins=t.tolist())
p2 = make_plot("Computation Time Distribution", hist2, edges2, 'Computation Time (in seconds)','Frequency')

s = np.linspace(min(soln_sizes)- 1, max(soln_sizes)+1, 10)
hist3,edges3 = np.histogram(soln_sizes, density=False, bins=s.tolist())
p3 = make_plot("Solution Size Distribution", hist3, edges3, 'Number of Solutions','Frequency')
...
p4 = make_plot("Security Zone Size Distribution", hist4, edges4, 'Number of Zones','Frequency')
...
p5 = make_plot("Firewall Distribution", hist5, edges5, 'Number of Firewalls','Frequency')

ac = np.linspace(min(acl_dist)- 1, max(acl_dist)+1, 10)
hist6,edges6 = np.histogram(acl_dist, density=False, bins=ac.tolist())
p6 = make_plot("ACLs Distribution", hist6, edges6, 'Number of ACLs','Frequency')

r = np.linspace(min(res_scores), max(res_scores), 10)
hist7,edges7 = np.histogram(res_scores, density=False, bins=r.tolist())
p7 = make_plot("Resilience Metric Distribution", hist7, edges7, 'Resilience Score','Frequency')

l = np.linspace(min(lodf_scores), max(lodf_scores), 10)
hist8,edges8 = np.histogram(lodf_scores, density=False, bins=l.tolist())
p8 = make_plot("LODF Distribution", hist8, edges8, 'LODF score','Frequency')

show(gridplot([p1,p2,p3,p4,p5,p6,p7,p8], ncols=4, width=400, height=400, toolbar_location=None))
