...
pw_path = r"D:\Experiments\pymoo-master\Exp\TX2000\ACTIVSg2000_ControlCenterExperience.pwb"

class OptimalZonation(Problem):
    def __init__(self,_G,sn_id,_branch_lodf, _prob_id, _combo, _consider_lodf=False, partition_size_upper = 40, partition_size_lower=0, min_nodes_partition = 1):
        self.G = _G
        ...
        nx.write_gpickle(self.G, str(_prob_id)+'_objective_var.gpickle')

        super().__init__(n_var=len(list(self.G.edges)), n_obj=3, xl=0, xu=1, type_var=int)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        obj_considered = []
        if 0 in self.combo:
            obj_considered.append(np.array(f1))
        if 1 in self.combo:
            obj_considered.append(np.array(f12))
        if 2 in self.combo:
            obj_considered.append(np.array(f2))
        if 3 in self.combo:
            obj_considered.append(np.array(f3))
        out["F"] = anp.column_stack(obj_considered)
...
for k,sg in sg_info.items():
    if len(sg.nodes()) > 10:
        obj_list = [0,1,2,3]
        r=3
        combos = list(combinations(obj_list,r))
        for combo in combos:
            nsga2_alg = NSGA2(
                pop_size=200,
                n_offsprings=20,
            ...
            gp_problem = OptimalZonation(_G=sg,sn_id=subname_id,_branch_lodf=branch_lodf, _prob_id=k,_consider_lodf=withLodf, _combo=combo)
            start = time.time()
            ...
            if res_gp_mo.X is not None:
                ...
                scipy.io.savemat('Utility_'+str(k)+'_size'+str(len(sg.nodes()))+'_obj'+str('.'.join(list(map(str, combo))))+'.mat', results)
        break
