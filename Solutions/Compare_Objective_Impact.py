...
import simplejson as json
from Problems.OptimalZone_ObjectiveImpact import OptimalZonation
    ...
    for k, sg in sg_info.items():
        if len(sg.nodes()) > 10:
            ...
            for combo in combos:
                ...
                gp_problem = OptimalZonation(_G=sg, sn_id=subname_id, _branch_lodf=branch_lodf, _prob_id=k,
                                             _consider_lodf=withLodf, _combo=combo)
                start = time.time()
                ...
                if res_gp_mo.X is not None:
                    ...
                    scipy.io.savemat('Utility_' + str(k) + '_size' + str(len(sg.nodes())) + '_obj' + str(
                        '.'.join(list(map(str, combo)))) + '.mat', results)
            break
